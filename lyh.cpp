...
//lostime 货物消失时间
// g++ -O2 lyh.cpp -o lyh 
// ./PreliminaryJudge -s 0 -f 0 -d input.txt -m maps/map1.txt "./lyh.exe"
// g++ -O2 lyh.cpp -o lyh 
// ./PreliminaryJudge -s 0 -f 0 -d input.txt -m maps/map-3.12.txt "./lyh.exe"
// g++ -O2 main.cpp -o main 
// ./PreliminaryJudge -s 0 -f 0 -d input.txt -m maps/map-3.12.txt "./main.exe"
int d_list[mxgood_num],d0;
int notdel[berth_num];
...
struct Robot
{
    ...
    void getmap(int id = -1)
    {
        ...
        {
            if((this->t) < mxgood_num)//说明和货物不连通,泊位不联通就不管了先 
            {
                this->t = -1;
                f[this->x][this->y] = -1;
            }
...
void Init()
{
    ...
    {
        int id;
        scanf("%d", &id);// 港口坐标和id不变，loading+time会变 
...
void vf_calc()
{
    ...
    if(onmap[j].ddl>zid && di[i][j]+dib[j][berth_num] < mapsize*2)
    {
        ...
        {
            // vf[i][j] = 1e5*(db)onmap[j].v / power(di[i][j]+dib[j][berth_num],param.alp,1) / power(onmap[j].ddl-zid,param.dying,2);
            // if(zid > param.rushtime) vf[i][j] = (db)onmap[j].v / power(di[i][j],param.alp,1)  / power(dib[j][berth_num],3);
            // else vf[i][j] = (db)onmap[j].v / power(di[i][j],param.alp,1) / power(onmap[j].ddl-zid,param.dying,2) / power(dib[j][berth_num],3);
            vf[i][j] = (db)onmap[j].v / power(di[i][j]+dib[j][berth_num],param.alp,1);
...
void getroute()
{
    ...
    //因为是逐个机器人getmap ，优先满足序号较早的需求 和旧的需求 
    // 旧的未满足的需求怎么处理？ 检查f[robot[i].x][robot[i].y] 即可知道“是否通路”
    
    fo(i,0,robot_num-1)
    if(robot[i].t!=-1)
    { //运行
        int t = robot[i].t,flag = 0;
        // if(robot[i].x >= onmap[t].x -2 &&robot[i].x <= onmap[t].x + 2 &&robot[i].y >= onmap[t].y -2 &&robot[i].y <= onmap[t].y + 2 && param.tiaoshi)
        //     printf("nearnear");
...
int main()
{
    ...
    if(param.out_sw)
    {
        ...
        printf("总掉落%d\n港口：%d\n上船：%d\n障碍：%d\n",cnt_gd,cnt_berth,cnt_boat,cnt_bar);
        fclose(stdout);
    }
    return 0;
}
