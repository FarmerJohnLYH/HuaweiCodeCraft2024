...
int mp[n][n][5];//[0,1,2,3] 0，港口，机器人，货物编号
...
int simple_dist(int x1,int y1,int x2,int y2)
{
...
int check(int x,int y,int crush,int rid)
    {
        if(x < 0 || x >= n || y < 0 || y >= n) return 0;
        if(ch[x][y] == '#') return 0;
        if(crush && mp[x][y][2] != -1 )
        {
            if(mp[x][y][2] == rid) return 1;
            int c = mp[x][y][2]; // 另一机器人id
            int tm = robot[c].d[robot[c].x][robot[c].y] - robot[c].d[x][y];// 物品到robot总时间 - 物品到(x,y)时间 
            int tm2 = simple_dist(robot[rid].x,robot[rid].y,x,y);
            if(abs(tm - tm2) > 10) return 1;
            return 0;//-1 可通过
        }
        return 1;
    }
