...
struct Param{
    ...
    int tiaoshi,footprint,out_sw,rushtime ;
    Param() 
    {
        alp = 0.6182;
        pred = 0.1352;
...
int main(int argc, char* argv[])
{
    ...
    db alp = stod(argv[2]);//0~1
    db dying = stod(argv[3]);//0~1,alp±0.2
    db pred = stod(argv[4]); //0~2
    db b_broadcast = stod(argv[5]); //1~2
    param.alp = alp;
    param.dying = dying;
    param.pred = pred;
    param.b_broadcast = b_broadcast;
    ...
    freopen(outputFile.c_str(), "a", stdout); // 会将数据添加到文件的末尾，而不是覆盖文件的原有内容。
    printf("实际参数%.4lf,%.4lf,%.4lf,%.4lf\n",param.alp,param.dying,param.pred ,param.b_broadcast);
